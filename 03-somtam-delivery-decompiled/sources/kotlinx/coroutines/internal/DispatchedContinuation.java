...
public final class DispatchedContinuation<T> extends DispatchedTask<T> implements CoroutineStackFrame, Continuation<T> {
    ...
    public void resumeWith(Object result) {
        ...
        if (this.dispatcher.isDispatchNeeded(context)) {
            this._state = state;
            this.resumeMode = 0;
            this.dispatcher.mo1594dispatch(context, this);
            return;
        }
        if (DebugKt.getASSERTIONS_ENABLED()) {
        }
    ...
    */
    public final void resumeCancellableWith(Object result, Function1<? super Throwable, Unit> onCancellation) {
        ...
        if (this.dispatcher.isDispatchNeeded(get$context())) {
            this._state = state;
            this.resumeMode = 1;
            this.dispatcher.mo1594dispatch(get$context(), this);
            return;
        }
        if (DebugKt.getASSERTIONS_ENABLED()) {
        }
            ...
            if (job$iv != null) {
                try {
                    if (job$iv.isActive()) {
                        ...
                        resumeWith(Result.m42constructorimpl(ResultKt.createFailure(cause$iv)));
                        z = true;
                    }
                    try {
                        if (!z) {
                            ...
                            CoroutineContext context$iv$iv = continuation.get$context();
    ...
    public final boolean resumeCancelled(Object state) {
        ...
        if (job != null && !job.isActive()) {
            ...
            Result.Companion companion = Result.INSTANCE;
            resumeWith(Result.m42constructorimpl(ResultKt.createFailure(cause)));
            return true;
        }
        return false;
    }
