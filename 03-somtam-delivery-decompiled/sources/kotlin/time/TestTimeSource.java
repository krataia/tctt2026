...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m1504plusAssignLRDsOJo(long duration) {
        long longDelta = Duration.m1414toLongimpl(duration, getUnit());
        ...
        if ($i$f$isSaturated == 0) {
            ...
            if ((this.reading ^ longDelta) >= 0 && (this.reading ^ newReading) < 0) {
                m1503overflowLRDsOJo(duration);
            }
            this.reading = newReading;
            return;
        }
        long half = Duration.m1371divUwyO8pc(duration, 2);
        long $this$isSaturated$iv = Duration.m1414toLongimpl(half, getUnit());
        if (!((($this$isSaturated$iv - 1) | 1) == Long.MAX_VALUE)) {
            long readingBefore = this.reading;
            try {
                m1504plusAssignLRDsOJo(half);
                m1504plusAssignLRDsOJo(Duration.m1403minusLRDsOJo(duration, half));
                return;
    ...
    private final void m1503overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + DurationUnitKt.shortName(getUnit()) + " is advanced by " + ((Object) Duration.m1417toStringimpl(duration)) + '.');
    }
}
