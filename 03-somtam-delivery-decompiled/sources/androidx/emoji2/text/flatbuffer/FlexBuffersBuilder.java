...
public class FlexBuffersBuilder {
    ...
    public int putBlob(String key, byte[] val) {
        int iKey = putKey(key);
        Value value = writeBlob(iKey, val, 25, $assertionsDisabled);
        this.stack.add(value);
        return (int) value.iValue;
    }
    ...
    public int endMap(String key, int start) {
        ...
        Value vec = createVector(iKey, start, this.stack.size() - start, $assertionsDisabled, $assertionsDisabled, keys);
        while (this.stack.size() > start) {
            this.stack.remove(this.stack.size() - 1);
        }
        this.stack.add(vec);
