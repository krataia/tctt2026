...
public class MotionLabel extends View implements FloatLayout {
    ...
    private void updateShaderMatrix() {
        ...
        float sh2 = Float.isNaN(this.mTextureHeight) ? this.mFloatHeight : this.mTextureHeight;
        float scale = (iw * sh2 < ih * sw ? sw / iw : sh2 / ih) * zoom;
        this.mTextShaderMatrix.postScale(scale, scale);
        float gapx = sw - (scale * iw);
        float gapy = sh2 - (scale * ih);
        ...
        float ty = (((panY * gapy) + sh2) - (scale * ih)) * 0.5f;
        this.mTextShaderMatrix.postTranslate(tx, ty);
        float panX2 = sw / 2.0f;
        this.mTextShaderMatrix.postRotate(rota, panX2, sh2 / 2.0f);
