...
public class Flow extends VirtualLayout {
    ...
    */
    public void measure(int widthMode, int widthSize, int heightMode, int heightSize) {
        ...
        needsCallbackFromSolver(this.mWidgetsCount > 0 ? c : 0);
    }
    ...
    private void measureChainWrap_new(ConstraintWidget[] widgets, int count, int orientation, int max, int[] measured) {
        ...
        if (orientation == 0) {
            ...
            while (i < count) {
                ...
                boolean doWrap3 = (width == max || (this.mHorizontalGap + width) + w > max) && list3.biggest != null;
                if (!doWrap3 && i > 0 && this.mMaxElementsWrap > 0 && col2 > this.mMaxElementsWrap) {
                    doWrap2 = true;
                } else {
                    doWrap2 = doWrap3;
                }
            ...
            while (i2 < count) {
                ...
                boolean doWrap4 = (height == max || (this.mVerticalGap + height) + h > max) && list5.biggest != null;
                if (!doWrap4 && i2 > 0 && this.mMaxElementsWrap > 0 && row > this.mMaxElementsWrap) {
                    doWrap = true;
                } else {
                    doWrap = doWrap4;
                }
