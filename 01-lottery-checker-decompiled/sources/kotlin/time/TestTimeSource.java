...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m1623plusAssignLRDsOJo(long duration) {
        long longDelta = Duration.m1533toLongimpl(duration, getUnit());
        ...
        if ($i$f$isSaturated == 0) {
            ...
            if ((this.reading ^ longDelta) >= 0 && (this.reading ^ newReading) < 0) {
                m1622overflowLRDsOJo(duration);
            }
            this.reading = newReading;
            return;
        }
        long half = Duration.m1490divUwyO8pc(duration, 2);
        long $this$isSaturated$iv = Duration.m1533toLongimpl(half, getUnit());
        if (!((($this$isSaturated$iv - 1) | 1) == Long.MAX_VALUE)) {
            long readingBefore = this.reading;
            try {
                m1623plusAssignLRDsOJo(half);
                m1623plusAssignLRDsOJo(Duration.m1522minusLRDsOJo(duration, half));
                return;
    ...
    private final void m1622overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + DurationUnitKt.shortName(getUnit()) + " is advanced by " + ((Object) Duration.m1536toStringimpl(duration)) + '.');
    }
}
