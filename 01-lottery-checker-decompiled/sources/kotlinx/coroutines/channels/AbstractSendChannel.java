...
public abstract class AbstractSendChannel<E> implements SendChannel<E> {
    ...
    public final Object mo1667trySendJP2dKIU(E element) {
        Object result = offerInternal(element);
        if (result == AbstractChannelKt.OFFER_SUCCESS) {
            return ChannelResult.INSTANCE.m1687successJP2dKIU(Unit.INSTANCE);
        }
        if (result == AbstractChannelKt.OFFER_FAILED) {
            Closed<?> closedForSend = getClosedForSend();
            return closedForSend == null ? ChannelResult.INSTANCE.m1686failurePtdJZtk() : ChannelResult.INSTANCE.m1685closedJP2dKIU(helpCloseAndGetSendException(closedForSend));
        }
        if (result instanceof Closed) {
            return ChannelResult.INSTANCE.m1685closedJP2dKIU(helpCloseAndGetSendException((Closed) result));
        }
    ...
    */
        ...
        while (true) {
            if (isFullImpl()) {
                ...
                if (offerResult != AbstractChannelKt.OFFER_SUCCESS) {
                    ...
                    cont.resumeWith(Result.m161constructorimpl(Unit.INSTANCE));
                    break;
                }
    ...
    public final void helpCloseAndResumeWithSendException(Continuation<?> continuation, E e, Closed<?> closed) {
        ...
        if (function1 != null && (it = OnUndeliveredElementKt.callUndeliveredElementCatchingException$default(function1, e, null, 2, null)) != null) {
            ...
            Result.Companion companion = Result.INSTANCE;
            continuation.resumeWith(Result.m161constructorimpl(ResultKt.createFailure(it)));
        } else {
            Result.Companion companion2 = Result.INSTANCE;
            continuation.resumeWith(Result.m161constructorimpl(ResultKt.createFailure(sendException)));
        }
    }
    ...
    */
    protected Object enqueueSend(Send send) {
        ...
        LockFreeLinkedListNode.CondAddOp condAdd$iv = new LockFreeLinkedListNode.CondAddOp(send2) { // from class: kotlinx.coroutines.channels.AbstractSendChannel$enqueueSend$$inlined$addLastIfPrevAndIf$1
    ...
    private final void helpClose(Closed<?> closed) {
        Object closedList = InlineList.m1704constructorimpl$default(null, 1, null);
        while (true) {
            LockFreeLinkedListNode prevNode = closed.getPrevNode();
            Receive previous = prevNode instanceof Receive ? (Receive) prevNode : null;
            if (previous == null) {
                break;
            } else if (!previous.mo1712remove()) {
                previous.helpRemove();
            } else {
                closedList = InlineList.m1709plusFjFbRPM(closedList, previous);
            }
        }
        if (closedList != null) {
