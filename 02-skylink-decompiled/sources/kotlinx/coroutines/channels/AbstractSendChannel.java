...
public abstract class AbstractSendChannel<E> implements SendChannel<E> {
    ...
    public final Object mo1530trySendJP2dKIU(E element) {
        ...
        if (result == AbstractChannelKt.OFFER_FAILED) {
            ...
            return closedForSend == null ? ChannelResult.INSTANCE.m1549failurePtdJZtk() : ChannelResult.INSTANCE.m1548closedJP2dKIU(helpCloseAndGetSendException(closedForSend));
        }
        if (result instanceof Closed) {
            return ChannelResult.INSTANCE.m1548closedJP2dKIU(helpCloseAndGetSendException((Closed) result));
        }
    ...
    */
    ...
    private final void helpClose(Closed<?> closed) {
        Object closedList = InlineList.m1567constructorimpl$default(null, 1, null);
        while (true) {
            LockFreeLinkedListNode prevNode = closed.getPrevNode();
            Receive previous = prevNode instanceof Receive ? (Receive) prevNode : null;
            if (previous == null) {
                break;
            } else if (!previous.mo1575remove()) {
                previous.helpRemove();
            } else {
                closedList = InlineList.m1572plusFjFbRPM(closedList, previous);
            }
        }
        if (closedList != null) {
