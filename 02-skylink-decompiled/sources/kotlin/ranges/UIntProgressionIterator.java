...
final class UIntProgressionIterator implements Iterator<UInt>, KMappedMarker {
    ...
    private UIntProgressionIterator(int first, int last, int step) {
        ...
        boolean z = true;
        int iUintCompare = UnsignedKt.uintCompare(first, last);
        if (step <= 0 ? iUintCompare < 0 : iUintCompare > 0) {
            z = false;
        }
        this.hasNext = z;
        this.step = UInt.m136constructorimpl(step);
        this.next = this.hasNext ? first : this.finalElement;
    }

    @Override // java.util.Iterator
    public /* bridge */ /* synthetic */ UInt next() {
        return UInt.m130boximpl(m1274nextpVg5ArA());
    }
    ...
    public int m1274nextpVg5ArA() {
        ...
        if (value == this.finalElement) {
            ...
            this.next = UInt.m136constructorimpl(this.next + this.step);
        }
        return value;
    }
}
