...
public final class LongSaturatedMathKt {
    ...
    public static final long m1484saturatingAddpTJri5U(long longNs, long duration) {
        long durationNs = Duration.m1376getInWholeNanosecondsimpl(duration);
        if (((longNs - 1) | 1) == Long.MAX_VALUE) {
            return m1483checkInfiniteSumDefinedPjuGub4(longNs, duration, durationNs);
        }
        if ((1 | (durationNs - 1)) == Long.MAX_VALUE) {
            return m1485saturatingAddInHalvespTJri5U(longNs, duration);
        }
    ...
    private static final long m1483checkInfiniteSumDefinedPjuGub4(long longNs, long duration, long durationNs) {
        if (!Duration.m1388isInfiniteimpl(duration) || (longNs ^ durationNs) >= 0) {
            return longNs;
        }
    ...
    private static final long m1485saturatingAddInHalvespTJri5U(long longNs, long duration) {
        long half = Duration.m1359divUwyO8pc(duration, 2);
        if (((Duration.m1376getInWholeNanosecondsimpl(half) - 1) | 1) == Long.MAX_VALUE) {
            return (long) (longNs + Duration.m1399toDoubleimpl(duration, DurationUnit.NANOSECONDS));
        }
        return m1484saturatingAddpTJri5U(m1484saturatingAddpTJri5U(longNs, half), half);
    }

    public static final long saturatingDiff(long valueNs, long originNs) {
        if ((1 | (originNs - 1)) == Long.MAX_VALUE) {
            return Duration.m1408unaryMinusUwyO8pc(DurationKt.toDuration(originNs, DurationUnit.DAYS));
        }
        ...
        if (((result ^ valueNs) & (~(result ^ originNs))) < 0) {
            ...
            return Duration.m1392plusLRDsOJo(duration, DurationKt.toDuration(resultNs, DurationUnit.NANOSECONDS));
        }
