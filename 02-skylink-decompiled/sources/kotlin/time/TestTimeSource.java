...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m1490plusAssignLRDsOJo(long duration) {
        long newReading;
        long longDelta = Duration.m1402toLongimpl(duration, getUnit());
        if (longDelta != Long.MIN_VALUE && longDelta != Long.MAX_VALUE) {
            ...
            if ((this.reading ^ longDelta) >= 0 && (this.reading ^ newReading) < 0) {
                m1489overflowLRDsOJo(duration);
            }
        } else {
            double delta = Duration.m1399toDoubleimpl(duration, getUnit());
    ...
    private final void m1489overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + "ns is advanced by " + ((Object) Duration.m1405toStringimpl(duration)) + '.');
    }
}
